...
2. **DHCP (动态主机配置协议全景时序与两次 ARP 探活)**：
   * 基于 **UDP** 的应用层协议（服务端端口 67，客户端端口 68）；
   * **四步广播获取租约**：`DHCPDISCOVER` (广播寻址) $\to$ `DHCPOFFER` (广播提供) $\to$ `DHCPREQUEST` (广播确认并通知其他服务器回收) $\to$ `DHCPACK` (广播确认)；
   * 💡 **两次 ARP 探活考点**：服务器在发送 `OFFER` 前用 ARP 探活确保未被静态占用；客户机在收到 `ACK` 后发免费 ARP 测冲突（若冲突发 `DHCPDECLINE` 谢绝并重发 Discover）；
   * 💡 **租期续约与释放**：0.5 倍租期 (T1) 单播 `REQUEST` 续约；0.875 倍租期 (T2) 广播求援；到期强制停用；`RELEASE` 单播主动释放。
