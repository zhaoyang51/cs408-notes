# 第6章 应用层 (HTTP / DNS / FTP / 邮件 / DHCP)

<span class="badge badge-freq">📊 考频 28 次（HTTP 协议 9 / DNS 解析 5 / FTP 传输 5 / 电子邮件 5 / DHCP 4）</span>
...
- **最少查询次数 (0 次)**：若主机本地 DNS 缓存命中，或本地域名服务器自身缓存命中，均**无需向外部发出任何查询**，最少为 **0 次**。
- **最多查询次数 (迭代解析逐级 4 次)**：根域名服务器 → 顶级域名服务器 (`.com`) → 权限域名服务器 (`xyz.com`) → 权限域名服务器 (`abc.xyz.com`)，共需发出 **4 次** 查询。
...
1. **非持续连接 (Non-persistent)**：总耗时 $= 2(1 + n)\text{ RTT}$；
2. **持续非流水线连接 (Persistent Non-pipelined)**：总耗时 $= (2 + n)\text{ RTT}$；
3. **持续流水线连接 (Persistent Pipelined)**：总耗时 $= 3\text{ RTT}$（与引用对象数 $n$ 无关）。
...
<KP author="Zhao" title="📡 DHCP 动态主机配置协议时序全景 · 两次 ARP 探活校验 · 租期续约生命周期" tag="核心考点">
<DhcpProtocolSimulator />
</KP>

### 5.1 DHCP 体系架构与协议层级辨析 (408 核心选择题)
- **协议层次**：DHCP 报文由传输层 **UDP 协议**进行承载，因而在体系结构上属于**应用层协议**；
- **端口分配**：
  - **DHCP 服务器**：监听 **UDP 67** 端口；
  - **DHCP 客户机**：监听 **UDP 68** 端口。
- **4 步获取租约**：
  1. `DHCPDISCOVER`：客户机广播寻址（源 `0.0.0.0:68` $\to$ 目的 `255.255.255.255:67`）；
  2. `DHCPOFFER`：服务器广播提供（源 `服务器IP:67` $\to$ 目的 `255.255.255.255:68`，**发前先用 ARP 探活**）；
  3. `DHCPREQUEST`：客户机广播确认（源 `0.0.0.0:68` $\to$ 目的 `255.255.255.255:67`，广播告知选中服务器并通知其他服务器回收预留）；
  4. `DHCPACK`：服务器广播确认（源 `服务器IP:67` $\to$ 目的 `255.255.255.255:68`，**收后客户机用免费 ARP 测冲突，若冲突发 DHCPDECLINE**）。
- **租约续约**：$0.5$ 倍租期（T1）单播 `DHCPREQUEST`；$0.875$ 倍租期（T2）广播 `DHCPREQUEST` 求援；到期强制停用；$RELEASE$ 主动释放。
