...
<KP author="Zhao" title="⚡ UDP 协议首部、伪首部与反码求和（2024 题 39 真题精析）" tag="核心考点">
<UdpChecksumCalculator />
</KP>

### 1.1 TCP vs UDP 核心特性对比表

| 特性维度 | TCP (传输控制协议) | UDP (用户数据报协议) |
|:---|:---|:---|
| **连接状态** | **面向连接**（三次握手、四次挥手） | **无连接**（发送前无需握手） |
| **可靠性** | **可靠交付**（确认重传、滑动窗口、保序去重） | **尽最大努力交付**（不可靠） |
| **传输形式** | **面向字节流**（将数据视为无结构字节流） | **面向报文**（保留应用层边界，不拆分不合并） |
| **通信方式** | 严格点对点单播（一对一） | 支持单播、多播 (组播)、广播（一对多、多对多） |
| **头部开销** | 最小 20 字节，最大 60 字节 | 固定 **8 字节**（源端口/目的端口/长度/校验和 各 2B） |
