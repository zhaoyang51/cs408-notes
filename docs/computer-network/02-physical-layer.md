...
- **核心特点**：
  - 检错能力极强（100% 检出单比特错、双比特错、奇数个错以及所有长度 $\le r$ 的突发错）。由硬件生成，速度极快。**仅用于检错丢弃，本身无纠错能力**（需上层配合 ARQ 协议实现可靠传输）；
  - **生成多项式约束（408 选择题常考）**：生成多项式 $G(X)$ **必须包含常数项 $1$**（即最低位为 $1$，最高位也为 $1$，保证能检测出所有的单比特差错）。
- **计算 5 步法**：
