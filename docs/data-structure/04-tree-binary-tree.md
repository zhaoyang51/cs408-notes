# 第4章 树和二叉树

<span class="badge badge-star">⭐️ 408 必考综合大题 (15分)</span>
<span class="badge badge-freq">📊 考频 ★★★★★</span>

---

## 1. 二叉树 5 大核心性质

<div class="formula-card">
  <div class="title">💡 二叉树核心性质与公式速查</div>
  <ul>
    <li><strong>叶子与双分支结点关系</strong>：$n_0 = n_2 + 1$（对任意二叉树恒成立）。</li>
    <li><strong>第 $i$ 层最大结点数</strong>：$2^{i-1}$。</li>
    <li><strong>高度为 $h$ 的最大总结点数</strong>：$2^h - 1$。</li>
    <li><strong>完全二叉树高度</strong>：$h = \lfloor \log_2 n \rfloor + 1 = \lceil \log_2(n+1) \rceil$。</li>
    <li><strong>完全二叉树编号关系</strong>：结点 $i$ 的左孩子为 $2i$，右孩子为 $2i+1$，父结点为 $\lfloor i/2 \rfloor$。</li>
  </ul>
</div>

---

## 2. 线索二叉树性质

- $n$ 个结点的二叉链表共有 $2n$ 个指针域，其中 **$n+1$ 个为空指针域**（用于构建线索）。
- **线索规则**：
  - `ltag == 1`：`lchild` 指向**前驱结点**；
  - `rtag == 1`：`rchild` 指向**后继结点**。

---

## 3. 哈夫曼树（最优二叉树）与并查集

| 概念 | 核心性质 | 计算公式 / 算法步骤 |
|:---|:---|:---|
| **哈夫曼树** | 只有度为 0 和度为 2 的结点（无度为 1 结点） | $n$ 个叶子总结点数 $= 2n - 1$ |
| **带权路径长度 (WPL)** | 叶子结点权值 $\times$ 到根路径长度 | $WPL = \sum (w_i \times l_i)$ |
| **并查集 (Union-Find)** | 双亲表示法树结构 | Find 路径压缩 + Union 小树并入大树（近似 $O(1)$） |

<details class="self-test">
  <summary>
    <span class="badge badge-star">⭐️ 考点自测</span>
    <span>权值为 $\{2, 3, 5, 7, 8\}$ 的叶子结点构造哈夫曼树，其 WPL 是多少？</span>
  </summary>
  <div class="answer-content">
    <p><strong>【合并过程】</strong>：</p>
    <ol>
      <li>选 2, 3 合并生成 5（分支和 5）</li>
      <li>选 5, 5 合并生成 10（分支和 10）</li>
      <li>选 7, 8 合并生成 15（分支和 15）</li>
      <li>选 10, 15 合并生成 25（分支和 25）</li>
    </ol>
    <p><strong>【WPL 极速算法】</strong>：所有非叶结点的权值之和 $= 5 + 10 + 15 + 25 = 55$。</p>
  </div>
</details>
